export {} //Lazm 34an a3ml scope mo5tlf 3n index.js
...
let toDoTasksArray : newTask[]= [];
let progressTasksArray : newTask[] = [];
let completeTasksArray : newTask[] = [];
...
function clickOnButtons(){
  ...
  if (!btn) return;
  let index: number = Number((btn as HTMLButtonElement).dataset.index);
  let fromName: string = (btn as HTMLButtonElement).dataset.type!;
  let btnAction: string = (btn as HTMLButtonElement).dataset.action!;
  if (btnAction === 'toDo') {
...
function getModalTemplate(task?: newTask): string {
  return `
...
function setupModalListeners(editIndex?: number, editFromName?: string): void {
  const taskName = document.getElementById("taskName") as HTMLInputElement;
  const taskPriority = document.getElementById("priority") as HTMLInputElement;
...
function showToDoTasks(){
  let toDoTasksNumber: number = toDoTasksArray.length;
...
function showProgressTasks(){
   let progressTasksNumber: number = progressTasksArray.length;
...
function showCompleteTasks(){
  let completeTasksNumber: number = completeTasksArray.length;
...
function getArrayByName(fromName:string){
  if (fromName === 'toDo') return { array: toDoTasksArray, key: 'toDoTasksArray' };
  if (fromName === 'progress') return { array: progressTasksArray, key: 'progressTasksArray' };
  if (fromName === 'complete') return { array: completeTasksArray, key: 'completeTasksArray' };
}
...
function addToCompleteTasksArray(cardIndex: number, fromName: string){
...
function addToProgressTasksArray(cardIndex: number, fromName: string){
  const source = getArrayByName(fromName);
  const card = source!.array[cardIndex];
  if (!card) return;
...
function addToDoTasksArray(cardIndex: number, fromName: string){
  const source = getArrayByName(fromName);
  const card = source!.array[cardIndex];
  if (!card) return;
...
function updateTask(cardIndex: number, fromName: string): void {
  const source = getArrayByName(fromName);
...
function deleteTask(cardIndex: number, fromName: string){
  const source = getArrayByName(fromName);
  const card = source!.array[cardIndex];
  if (!card) return;
  source!.array.splice(cardIndex, 1);
